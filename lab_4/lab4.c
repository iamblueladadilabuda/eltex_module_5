...
static ssize_t device_write(struct file *filp, const char __user *buff, 
    ...
    if (len > BUF_LEN - 1) {
        pr_warn("Write length exceeds buffer size. Truncating to %d bytes.\n", BUF_LEN - 1);
        len = BUF_LEN - 1;
    }
    
    /* Копируем данные из пользовательского пространства в ядро */
    if (copy_from_user(msg, buff, len)) {
        pr_alert("Failed to copy data from user space\n");
        return -EFAULT;
    }
    
    bytes_written = len;
    
    /* Добавляем нулевой терминатор для строки */
    msg[len] = '\0';
    
    pr_info("Received %d bytes from user: %s\n", bytes_written, msg);
